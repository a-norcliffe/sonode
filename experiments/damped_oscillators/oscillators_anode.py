...
class ODEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.empty((len(samp_ts), cutoff, 2))
        for i in range(len(samp_ts)):
            out[i] = integrated[i].gather(1, self.indices)
        return out
...
if __name__ == '__main__':
    ...
    zeros = torch.zeros((len(z0), args.extra_dim))
    z0 = torch.cat((z0, zeros), dim=1).float().to(device)
