...
class ODEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.empty(args.ntimestamps, 3, 1)
...
def f3(t):
    return (e*torch.sin(t)+f*torch.cos(t))*torch.exp(-0.1667*t)
...
if __name__ == '__main__':
    ...
    # make data
    zeros = torch.zeros(3, args.extra_dim).float()
    z0 = torch.cat((torch.tensor([[f1(samp_ts[0])]\
                                   ,[f2(samp_ts[0])]\
                                   ,[f3(samp_ts[0])]\
                                   ]).float(), zeros), dim=1).to(device)
    z = torch.empty((args.ntimestamps, 3, 1)).float()
    for i in range(args.ntimestamps):
    ...
    if args.visualize:
        ...
        plt.plot(samp_ts, learnt_f3, color='#DDAA33', label='Learn $x_{3}$')
        ...
        learnt_a3 = np.empty((args.ntimestamps))
        for i in range(args.ntimestamps):
            learnt_a1[i] = pred_a[i][0]
            learnt_a2[i] = pred_a[i][1]
            learnt_a3[i] = pred_a[i][2]
