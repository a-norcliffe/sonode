...
parser.add_argument('--evenly_spread_timestamps', type=bool, default=True)
...
if __name__ == '__main__':
    ...
    feature_layers = [initial_velocity(dim, nhidden), ODEBlock(ODEfunc(dim, nhidden), samp_ts, ids)]
    ...
    results[0][0][int(10*args.noise)][int(args.experiment_no-1)] = loss
    ...
    zN = odeint(feature_layers[1].odefunc, y0, torch.tensor([t0, tN]).float())
    zN = zN[1]
    pred_z = odeint(feature_layers[1].odefunc, zN, test_ts)
    ids = torch.arange(data_dim)
    ids = ids.repeat(ntest, 1)
    pred_x_test = pred_z.gather(1, ids)
    test_loss = loss_func(pred_x_test, test_z).detach().numpy()
    results[1][0][int(10*args.noise)][int(args.experiment_no-1)] = test_loss
    ...
    if args.visualise:
        ...
        ts = ts.numpy()
        z_real = z_real.numpy()
        plt.plot(ts, z_real, color = 'b', label='True')
       ...
       # make indices for getting position
        ...
        y0 = feature_layers[0](z0)
        pred_z = odeint(feature_layers[1].odefunc, y0, learnt_ts)
        ...
        plt.ylabel('x')
        plt.xlabel('t')
        plt.title('SONODE Noise = '+str(args.noise)+' Experiment = '+str(args.experiment_no))
